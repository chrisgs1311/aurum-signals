...
def _worker_price():
    """Worker #1: precio cada 500ms — corazón del sistema."""
    while True:
        try:
            d = _fetch_with_retry("https://api.gold-api.com/price/XAU",
                                  timeout=5, retries=2, backoff=0.3)
            if d and "price" in d:
...
def _worker_ohlc():
    """Worker #2: OHLC cada 30s — alimenta motor SCALP con candles reales."""
    while True:
        try:
            for interval, tf, size in [("5min", "5m", 200), ("15min", "15m", 100)]:
                candles = get_historical_ohlc(interval, size)
                if candles:
                    key = f"ohlc_{tf.replace('m','min') if 'min' not in tf else tf}"
...
def start_all_workers():
    ...
    workers = [
        ("Price Feed (500ms)",  _worker_price),
        ("OHLC Feed (30s)",     _worker_ohlc),
...
def main():
    ...
    print(f"\n  ⚡ Latencia: precio 500ms | señal 2s | SSE push instantáneo")
    print("\n  Para detener: Ctrl + C")
    print("-" * 54)
