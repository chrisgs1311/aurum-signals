...
_data_lock = threading.Lock()  # protege price_history, *scalp_prices**, _ohlc_candles
...
key = f”{ev[‘name’]}*{ev[‘datetime_utc’].isoformat()}*{alert_min}”
...
<div class="sub">BACKTEST REAL · XAUUSD 5M · {d["data_from"]} → {d["data_to"]} · {d["candles"]} velas · {d["generated_at"]} UTC</div>
<div class="g4">
  <div class="card"><div class="cl">WIN RATE</div><div class="cv" style="color:{wrc}">{d["wr"]}%</div><div class="cs">{d["wins"]}W / {d["losses"]}L</div></div>
  <div class="card"><div class="cl">PROFIT FACTOR</div><div class="cv" style="color:{pfc}">{d["pf"]}</div><div class="cs">objetivo ≥ 1.5</div></div>
...
<div class="g4">
  ...
  <div class="card"><div class="cl">SHARPE</div><div class="cv" style="color:{'#00CC88' if d['sharpe']>=1 else '#CC3344'}">{d["sharpe"]}</div><div class="cs">objetivo ≥ 1.0</div></div>
...
<div class="g2">
  ...
  <div><div class="sec">SCORE TIERS</div><div class="panel" style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
    <div style="text-align:center;padding:14px;border:1px solid var(--border)"><div style="font-family:'Playfair Display',serif;font-size:26px;color:var(--gold)">{d["snipers"]}</div><div style="font-size:7px;letter-spacing:2px;color:var(--dim);margin-top:4px">SNIPER ≥85</div><div style="font-size:11px;margin-top:6px;color:{'#00CC88' if d['sn_wr']>=55 else '#CC3344'}">WR {d["sn_wr"]}%</div></div>
