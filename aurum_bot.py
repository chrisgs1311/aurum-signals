...
def push_price(p):
    with _data_lock:
        ...
        # v6.2: subido de 500 → 2000 para que la IA acumule más muestras con el tiempo
        if len(price_history) > 2000:
            del price_history[:-2000]

# ── FILTRO DE NOTICIAS ────────────────────────────────────
# (weekday, hour_utc, minute, nombre, impacto)
# v6.2: Calendario DINÁMICO desde Forex Factory (no hardcoded)
# El calendario hardcoded del v6.1 era falso (decía "todos los miércoles ADP", lo cual es incorrecto)
# Forex Factory publica un JSON semanal con eventos REALES por fecha exacta
_news_cache = {"events": [], "fetched_at": 0}
_news_alerted = set()  # tracking de eventos ya alertados (evita duplicados)
PAUSE_MINUTES_BEFORE = 30
PAUSE_MINUTES_AFTER  = 30
ALERT_MINUTES_BEFORE = [60, 15, 5]

# Eventos high-impact que afectan al oro (filtro por nombre)
...
def _fetch_forex_factory_calendar():
    """v6.2: Descarga calendario REAL de Forex Factory (JSON semanal).
    Retorna lista de eventos con fecha/hora real, no hardcoded."""
    try:
        ...
        for ev in data:
            ...
            # Filtrar: solo USD high-impact que afecten al oro
            if currency != "USD": continue
            if impact != "High": continue
            # Verificar que el título coincida con keywords relevantes
            if not any(kw.lower() in title.lower() for kw in HIGH_IMPACT_KEYWORDS):
                continue
            try:
                # Parse ISO datetime "2026-04-08T08:30:00-04:00"
...
def check_upcoming_news():
    """v6.2: detecta eventos próximos REALES (no hardcoded)."""
    now = _dt.datetime.utcnow()
    alerts = []
    for ev in _get_news_events():
        mins_until = (ev["datetime_utc"] - now).total_seconds() / 60
        # Verificar ventanas de alerta
...
def ai_train_if_needed(prices):
    """v6.2: usa closes de velas OHLC REALES (5m + 15m) como dataset principal.
    El parámetro `prices` (price_history tick) se usa solo como suplemento.
    Antes el AI se quedaba en ~455 muestras porque price_history estaba capado a 500 ticks."""
    global _ai_last_train, _ai_last_backup
    now = time.time()
    if now - _ai_last_train > 300:
        # v6.2: combinar velas OHLC reales (mejor calidad) + price_history como respaldo
        with _data_lock:
            ohlc5_closes  = [c["c"] for c in _ohlc_candles_5m]
            ohlc15_closes = [c["c"] for c in _ohlc_candles_15m]
        # Dataset combinado: prioriza velas reales 5m, agrega 15m y price_history
        combined = list(ohlc5_closes)
        if len(ohlc15_closes) > 0:
            combined = ohlc15_closes + combined  # 15m primero (más viejo), 5m al final
        if len(prices) > len(combined):
            # Si el tick history tiene más datos que las velas, usarlo como base
...
# ── WORKERS DE BAJA LATENCIA (v6.1) ──────────────────────
# Arquitectura: workers dedicados → cache RAM → endpoints instantáneos → SSE push

# Cache RAM — siempre fresco, 0 latencia en lectura
_live_cache = {
    "price": None,          # último precio gold-api
    "price_ts": 0,          # timestamp
    "ohlc_5m": [],          # candles 5min
    "ohlc_15m": [],         # candles 15min
    "ohlc_ts": 0,
    "signal": None,         # última señal SCALP v2
    "signal_ts": 0,
    "ai_prob": None,        # última predicción ML
    "ai_signal": None,
    "price_stale": False,   # v6.1: staleness flag
    "ohlc_stale": False,
    "price_fails": 0,       # v6.1: consecutive failures
}
_sse_clients = []           # lista de queues para SSE
_sse_lock = threading.Lock()

# v6.1: Staleness thresholds
STALE_PRICE_SEC = 30    # precio > 30s = stale
STALE_OHLC_SEC  = 180   # OHLC > 3min = stale

def is_data_stale():
    """v6.1: Verifica si los datos están viejos."""
    now = time.time()
...
# v6.4: Massive.com API (antes Polygon.io) — forex real-time, unlimited calls
        ...
        try:
            ...
            for a in alerts:
                ...
                if mins <= 5:
                    ...
                    urgency = "📅 *PROGRAMADA*"
                event_time_str = a["datetime_utc"].strftime("%H:%M UTC")
                msg = (
                    f"{urgency}\n\n"
                    f"{icon} *NOTICIA HIGH IMPACT*\n\n"
                    f"📌 Evento: *{a['name']}*\n"
                    f"⏰ Hora: {event_time_str}\n"
                    f"⏳ Faltan: *{mins} min*\n"
                    f"📊 Impacto: {a['impact']}\n\n"
                    f"━━━━━━━━━━━━━━━━━━\n"
...
# ═══════════════════════════════════════════════════════════════════
# PAPER TRADING SYSTEM v6.5
# Registra cada señal, simula ejecución con precio real, calcula P&L
# Persistente en /data/paper_trades.json
# ═══════════════════════════════════════════════════════════════════

# ═══════════════════════════════════════════════════════════════════
# RISK MANAGER v6.5
# Daily Loss Limit + Circuit Breaker para protección de prop firm
# ═══════════════════════════════════════════════════════════════════

RISK_STATE = {
    "daily_loss_limit_r": -3.0,      # -3R = daily loss (ajustable)
    "weekly_loss_limit_r": -8.0,     # -8R = weekly loss
    "max_consecutive_losses": 3,     # 3 pérdidas seguidas = pausa
    "circuit_breaker_hours": 4,      # pausa 4 horas después de 3 losses
    "paused_until": 0,               # timestamp hasta cuando está pausado
    "pause_reason": "",              # razón del bloqueo
}

def risk_get_today_stats():
    """Calcula stats del día actual (UTC)."""
    today = _dt.datetime.utcnow().strftime("%Y-%m-%d")
    with _paper_lock:
        trades_today = [t for t in _paper_trades 
                        if t["status"] == "CLOSED" 
                        and t.get("closed_at") 
    ...
    # Consecutive losses desde el último trade
    consec = 0
...
def risk_get_week_stats():
    """Calcula stats de los últimos 7 días."""
    now = time.time()
    week_ago = now - (7 * 86400)
    with _paper_lock:
        trades_week = [t for t in _paper_trades 
                       if t["status"] == "CLOSED" 
                       and t.get("closed_at", 0) > week_ago]
...
def risk_get_status():
    """Estado completo del risk manager para el /debug y dashboard."""
    day = risk_get_today_stats()
    week = risk_get_week_stats()
...
_paper_trades = []  # lista de trades abiertos/cerrados
_paper_lock = threading.Lock()

def _load_paper_trades():
    """Carga trades guardados del disco al iniciar."""
    global _paper_trades
    try:
        if os.path.exists(PAPER_TRADES_FILE):
            with open(PAPER_TRADES_FILE, "r") as f:
                _paper_trades = json.load(f)
...
def _save_paper_trades():
    """Guarda trades al disco (persistencia entre reinicios)."""
    try:
...
def paper_register_signal(signal_data, entry_price):
    """Registra una nueva señal como paper trade abierto."""
    trade = {
        ...
        "closed_at": None,
        "exit_price": None,
        "result": None,  # WIN / LOSS / BE / TIMEOUT
        "pnl_r": 0.0,
        "duration_sec": 0,
    }
    with _paper_lock:
    ...
    _save_paper_trades()
    print(f"  📝 Paper trade #{trade['id']}: {trade['direction']} @ ${entry_price:.2f}")
    return trade

def paper_check_open_trades(current_price):
    """Verifica cada trade abierto contra el precio actual.
    Cierra si toca TP, SL o expira (24h)."""
    ...
    with _paper_lock:
        for t in _paper_trades:
            ...
            entry = t["entry"]
            # Verificar TP/SL
            hit_tp = (is_buy and current_price >= tp) or (not is_buy and current_price <= tp)
            hit_sl = (is_buy and current_price <= sl) or (not is_buy and current_price >= sl)
            if hit_tp:
                t["status"] = "CLOSED"
                t["closed_at"] = now
                t["exit_price"] = tp
                ...
                # v7.1: alimentar rolling performance monitor automáticamente
            ...
            elif now - t["opened_at"] > 86400:  # 24h timeout
                t["status"] = "CLOSED"
                t["closed_at"] = now
                t["exit_price"] = current_price
                t["result"] = "TIMEOUT"
                # P&L proporcional a donde cerró
                if is_buy:
                    progress = (current_price - entry) / (tp - entry) if tp != entry else 0
                else:
                    progress = (entry - current_price) / (entry - tp) if tp != entry else 0
...
def paper_get_stats():
    """Calcula estadísticas completas del paper trading."""
    with _paper_lock:
    ...
    pf = round(win_r / loss_r, 2) if loss_r > 0 else (999 if win_r > 0 else 0)
    # Streaks
    max_w = max_l = cw = cl = 0
    cur = None
    for t in closed:
        r = t["result"]
        if r == "WIN":
            cw = cw + 1 if cur == "W" else 1
            cl = 0
            cur = "W"
            max_w = max(max_w, cw)
        elif r == "LOSS":
            cl = cl + 1 if cur == "L" else 1
            cw = 0
            cur = "L"
            max_l = max(max_l, cl)
    # Por sesión
    by_sess = {}
    for t in closed:
        s = t.get("session", "Unknown").split()[0] if t.get("session") else "Unknown"
        if s not in by_sess:
            by_sess[s] = {"n": 0, "w": 0, "r": 0.0}
        by_sess[s]["n"] += 1
        if t["result"] == "WIN":
            by_sess[s]["w"] += 1
        by_sess[s]["r"] += t["pnl_r"]
    ...
    for t in closed:
        ...
        if tier not in by_tier:
            by_tier[tier] = {"n": 0, "w": 0, "r": 0.0}
        by_tier[tier]["n"] += 1
        if t["result"] == "WIN":
            by_tier[tier]["w"] += 1
        by_tier[tier]["r"] += t["pnl_r"]
    for tier in by_tier:
        by_tier[tier]["wr"] = round(by_tier[tier]["w"] / by_tier[tier]["n"] * 100, 1)
        by_tier[tier]["r"] = round(by_tier[tier]["r"], 2)
    # Equity curve
    eq = [0.0]
    for t in closed:
        eq.append(round(eq[-1] + t["pnl_r"], 2))
    return {
        "total": len(all_trades),
        "open": len(opened),
        "closed": len(closed),
        ...
        "profit_factor": pf,
        "expectancy": round(total_r / len(closed), 3),
        "max_win_streak": max_w,
        "max_loss_streak": max_l,
        "best_trade": round(max(t["pnl_r"] for t in closed), 2),
        "worst_trade": round(min(t["pnl_r"] for t in closed), 2),
        "by_session": by_sess,
        "by_tier": by_tier,
        "equity_curve": eq,
...
def _worker_paper():
    """Worker #8: Verifica paper trades cada 1s usando precio live."""
    while True:
        try:
            live = _live_cache.get("price")
            if live and live.get("price"):
                paper_check_open_trades(live["price"])
        except Exception as e:
...
# ── v7: CORRELACIÓN DXY (Índice del Dólar) ───────────────
# Oro tiene correlación inversa histórica ~-0.85 con DXY
# Si DXY sube fuerte, oro suele caer (y viceversa)
_dxy_cache = {"trend": "neutral", "change_pct": 0.0, "last_update": 0}

def get_dxy_trend():
    """v7: obtiene tendencia de DXY para confirmar dirección de oro.
    Retorna: {"trend": "up"/"down"/"neutral", "change_pct": X.X}
    """
    now = time.time()
    # Cache 5 min
    if now - _dxy_cache["last_update"] < 300:
        return {"trend": _dxy_cache["trend"], "change_pct": _dxy_cache["change_pct"]}
    # Twelve Data soporta DXY
    try:
        ...
        print(f"  ⚠ DXY fetch: {e}")
    # Default neutral si falla
    _dxy_cache["last_update"] = now
    return {"trend": "neutral", "change_pct": 0.0}

def check_dxy_correlation(gold_direction):
    """v7: retorna dict con check de correlación inversa DXY.
    - Si gold quiere COMPRAR y DXY sube fuerte → MALO (conflicto)
    - Si gold quiere COMPRAR y DXY baja → BUENO (confirmación)
    - Si gold quiere VENDER y DXY sube → BUENO
    - Si gold quiere VENDER y DXY baja fuerte → MALO
    """
    dxy = get_dxy_trend()
...
# ── v7.1: US10Y YIELDS (filtro macro principal para oro) ─
# Cuando yields suben → holding oro cuesta más → oro baja
# Cuando yields bajan → oro atractivo → oro sube
_us10y_cache = {"trend": "neutral", "change_bps": 0.0, "last_update": 0, "level": 0.0}

def get_us10y_trend():
    """Obtiene tendencia de US 10Y Treasury yields."""
    now = time.time()
    ...
    try:
        # Twelve Data soporta US10Y como símbolo
        ...
        if data and "values" in data:
            ...
            if len(closes) >= 6:
                current = closes[-1]
                prev_6h = closes[-6]
                # Cambio en basis points (1 bps = 0.01%)
                change_bps = (current - prev_6h) * 100
                if change_bps > 3:  # yields subieron >3bps → malo para oro
                    trend = "rising"
                elif change_bps < -3:  # yields bajaron >3bps → bueno para oro
                    trend = "falling"
                else:
                    trend = "stable"
                _us10y_cache.update({
...
def check_yields_for_gold(gold_direction):
    """Verifica si US10Y yields confirman o contradicen la dirección de oro.
    - Yields rising + gold COMPRAR → conflicto (yields altos = oro baja)
    - Yields falling + gold COMPRAR → confirmación
    - Yields rising + gold VENDER → confirmación
    - Yields falling + gold VENDER → conflicto
    """
    y = get_us10y_trend()
    if gold_direction == "COMPRAR":
        if y["trend"] == "falling":
            return {"aligned": True, "bonus": 8,
                    "note": f"US10Y bajando ({y['change_bps']}bps) → favorece oro ↑"}
        elif y["trend"] == "rising" and abs(y["change_bps"]) > 5:
            return {"aligned": False, "bonus": -20,
                    "note": f"⚠ US10Y subiendo ({y['change_bps']}bps) → presiona oro ↓"}
    elif gold_direction == "VENDER":
        if y["trend"] == "rising":
            return {"aligned": True, "bonus": 8,
                    "note": f"US10Y subiendo ({y['change_bps']}bps) → favorece oro ↓"}
        elif y["trend"] == "falling" and abs(y["change_bps"]) > 5:
            return {"aligned": False, "bonus": -20,
                    "note": f"⚠ US10Y bajando ({y['change_bps']}bps) → presiona oro ↑"}
...
# ── SCORING SCALP v2 ─────────────────────────────────────
SCORE_SNIPER = 85   # entrada agresiva (sin cambios)
SCORE_NORMAL = 78   # v7: subido de 70 → 78 para reducir señales falsas
SCORE_EARLY  = 999  # v7: desactivado — causaba muchas señales falsas al entrar sin BOS

# ── PRE-SIGNAL STATE (v6.1) ──────────────────────────────
# Detecta condiciones previas a señal completa → alerta temprana
_pre_signal = {
    "state": "IDLE",       # IDLE → LIQUIDITY_NEAR → SWEEP_DETECTED → READY
    "bias": None,
    "sweep_level": 0,
    "fvg_zone": None,
    "ts": 0,
...
def detect_pre_signal(prices, candles=None):
    """v6.1: Detecta estados previos a señal para entrada temprana."""
    global _pre_signal
    ...
    # Paso 1: ¿Hay liquidez cerca? (swing highs/lows dentro de 0.15% del precio)
    if bias == "bullish":
    ...
    # Paso 2: ¿Hubo sweep? (ya procesado en detect_sweep)
...
# ── SWEEP QUALITY FILTER (v6.1) ──────────────────────────
def _is_real_sweep(prices, sweep_level, bias, candles=None, atr=None):
    """
    v6.1: Filtra micro-sweeps falsos.
    Un sweep REAL necesita:
    1. Penetración mínima (>30% del ATR)
    2. Displacement fuerte después (vela de impulso)
    3. No ser un sweep de continuación en contra del HTF
    """
    if not atr or atr <= 0:
        if len(prices) >= 15:
            atr = sum(abs(prices[i]-prices[i-1]) for i in range(len(prices)-14, len(prices))) / 14
        else:
            return True  # sin ATR no podemos filtrar, dejamos pasar

    price = prices[-1]

    # 1. PENETRACIÓN MÍNIMA — micro-sweeps de <30% ATR son ruido
    if bias == "bullish":
        penetration = sweep_level - min(prices[-5:])
    else:
        penetration = max(prices[-5:]) - sweep_level
    min_penetration = atr * 0.3
    if penetration < min_penetration:
        return False  # micro-sweep, ignorar

    # 2. DISPLACEMENT — después del sweep debe haber impulso fuerte
    # Mínimo: la vela post-sweep debe tener body > 50% de su rango
    if len(prices) >= 3:
        body = abs(prices[-1] - prices[-2])
        rng  = max(prices[-3:]) - min(prices[-3:]) + 1e-9
        displacement = body / rng
        if displacement < 0.35:
            return False  # sin desplazamiento, sweep débil

    # 3. DIRECCIÓN CORRECTA post-sweep
    if bias == "bullish" and prices[-1] < prices[-2]:
        return False  # sweep bullish pero precio sigue bajando
    if bias == "bearish" and prices[-1] > prices[-2]:
        return False  # sweep bearish pero precio sigue subiendo

    return True

def _classify_sweep_type(prices, bias, candles=None):
    """
    v6.1: Clasifica sweep como REVERSA o CONTINUACIÓN.
    
    REVERSA (lo que queremos):
    - Sweep contra tendencia previa → rechazo → reversa
    - Ejemplo: tendencia bajista → sweep de low → reversa alcista
    
    CONTINUACIÓN (trampa, evitar):
    - Sweep en dirección de tendencia → no hay reversa real
    - Ejemplo: tendencia alcista → sweep de high → continúa alcista (no es reversa)
    """
    if len(prices) < 20: return "UNKNOWN", 0.5

    # Tendencia previa: momentum de las últimas 10-15 velas ANTES del sweep
    pre_trend = prices[-10] - prices[-15] if len(prices) >= 15 else 0
    # Tendencia reciente: últimas 3-5 velas (post-sweep)
    post_trend = prices[-1] - prices[-3] if len(prices) >= 3 else 0

    if bias == "bullish":
        # Sweep bullish = sweep de lows
        # REVERSA: previa bajista + post alcista (lo bueno)
        # CONTINUACIÓN: previa alcista + post alcista (trampa)
        if pre_trend < 0 and post_trend > 0:
            confidence = min(1.0, abs(post_trend / (abs(pre_trend) + 1e-9)))
            return "REVERSA", round(confidence, 2)
        elif pre_trend > 0:
...
# ── v7: REGIME DETECTION AVANZADO ────────────────────────
def detect_market_regime(prices, n=30):
    """v7: Detecta el régimen actual del mercado y devuelve dict completo.
    
    Regímenes:
    - TRENDING_STRONG: tendencia clara + ADX alto, operar con la tendencia
    - TRENDING_WEAK: tendencia débil, ser selectivo
    - RANGING: mercado lateral, solo operar reversals en extremos
    - VOLATILE: volatilidad extrema (spike news), pausar
    - QUIET: volatilidad muy baja, pausar (spread vs movimiento malo)
    """
    if len(prices) < n:
    ...
    # Range vs ATR (trend strength proxy)
    rng = max(recent) - min(recent)
    rng_atr_ratio = rng / (atr + 1e-9)
    # Direccionalidad: cuánto del rango total corresponde a movimiento direccional
    ...
    # TRENDING_STRONG: direccionalidad alta + EMA slope fuerte
...
def get_ml_filter(prices, direction=None):
    """v7: filtro direccional — la IA filtra en contexto de la dirección del motor.
    
    Antes tenía bug: si motor quería VENDER y IA daba prob=0.3 (bajada esperada),
    el filtro devolvía 'LOW' y bloqueaba la señal. Ahora:
    - Si direction='COMPRAR': prob alta = HIGH, prob baja = LOW (bloquea)
    - Si direction='VENDER': prob baja = HIGH (IA confirma bajada), prob alta = LOW
    - Si direction=None (uso legacy): comportamiento viejo
    """
    if not _ai.trained or len(prices) < 35:
        return "MED", 0.5
    prob = _ai.predict_proba(prices)
    if prob is None: return "MED", 0.5
    # v7: ajustar prob según dirección
    if direction == "VENDER":
        # Para SELL, la IA debería predecir bajada: prob BAJA es bueno
        effective_prob = 1.0 - prob  # invertir para que "alto = bueno para esta dirección"
    else:
        # COMPRAR o None: comportamiento normal
        effective_prob = prob
...
# v6.2: tracking de tiempo para velas sintéticas
_last_5m_tick = 0
_last_15m_tick = 0

def update_ict_prices(price, candle=None):
    """Alimenta los historiales de precios para el motor.
    v6.2: velas 5m/15m basadas en TIEMPO REAL (segundos), no tick count.
    Con WebSocket a ~1 tick/seg, el modo tick count construía velas 5m cada 5 SEGUNDOS
    en vez de cada 5 MINUTOS, generando señales en ruido."""
    global _last_5m_tick, _last_15m_tick
    with _data_lock:
        _scalp_prices_1m.append(price)
        if len(_scalp_prices_1m) > 500:
            del _scalp_prices_1m[:-500]
        now = time.time()
        # 5m: agregar un close cada 300 segundos reales (no cada 5 ticks)
        if now - _last_5m_tick >= 300:
            _scalp_prices_5m.append(price)
            if len(_scalp_prices_5m) > 300:
                del _scalp_prices_5m[:-300]
            _last_5m_tick = now
        # 15m: cada 900 segundos reales
        if now - _last_15m_tick >= 900:
...
def run_ict_engine(current_price, atr):
    """
    AURUM SCALP v6.2 — Motor principal.
    v6.3: 100% datos reales. Si no hay velas OHLC reales suficientes → no opera.
    NO usa buffers sintéticos. NO aproxima nada. NO opera con datos delayed.
    Orden: Sweep(real) → BOS → FVG → EMA → ML → Score
    """
    global _last_scalp_signal
    import time as _t

    # v6.3: SOLO velas OHLC reales de Twelve Data. Sin fallback sintético.
    if len(_ohlc_candles_5m) < 20:
        return None  # sin datos reales suficientes → no opera
    if len(_ohlc_candles_15m) < 15:
        return None  # sin contexto 15m real → no opera

    ohlc5 = _ohlc_candles_5m
    ohlc15 = _ohlc_candles_15m

    # Closes de velas 5m REALES
    p5 = [c["c"] for c in ohlc5]
    # Precio live como "vela en formación" (es el precio REAL del WebSocket)
    if current_price and abs(current_price - p5[-1]) > 0.01:
        p5 = p5 + [current_price]

    # Closes de velas 15m REALES (sin fallback)
    p15 = [c["c"] for c in ohlc15]

    if not is_trading_session(): return None
    
    # v7: REGIME DETECTION (reemplaza detect_chop binario con 5 regímenes)
    regime_info = detect_market_regime(p5)
    if not regime_info["should_trade"]:
        return None  # QUIET o VOLATILE → pausar
    # Se usa más abajo para ajustar score

    # ═════════════════════════════════════════════════════
    # v6.5: MULTI-TIMEFRAME CONFLUENCE (filtro institucional)
    # Exige alineación 4H → 1H → 15M → 5M antes de operar
    # Esto elimina ~60-70% de falsas señales del motor base
    # ═════════════════════════════════════════════════════
    mtf_bias_4h = None
    mtf_bias_1h = None
    mtf_bias_15m = None
    ...
    # 4H bias — HTF macro direction
    if ohlc4h and len(ohlc4h) >= 21:
        closes_4h = [c["close"] for c in ohlc4h]
        mtf_bias_4h = get_ema_bias(closes_4h)
    
    # 1H bias — swing structure
    if ohlc1h and len(ohlc1h) >= 21:
        closes_1h = [c["close"] for c in ohlc1h]
        mtf_bias_1h = get_ema_bias(closes_1h)
    
    # 15M bias — entry structure
    mtf_bias_15m = get_ema_bias(p15)
    
    # CONFLUENCIA: contar alineación HTF
    bias_5m = get_ema_bias(p5)
    if not bias_5m: return None
    
    htf_biases = [b for b in [mtf_bias_4h, mtf_bias_1h, mtf_bias_15m] if b]
    
    # Debe haber al menos 2 de 3 HTFs disponibles y alineados con el 5m
    if len(htf_biases) >= 2:
        aligned = sum(1 for b in htf_biases if b == bias_5m)
        # Requerir al menos 2/3 alineados (o 2/2 si solo hay 2)
        min_aligned = 2 if len(htf_biases) == 3 else 2
        if aligned < min_aligned:
            # HTF no alineado con 5m → no operar
            return None
    # Si no hay suficientes HTF (< 2), el bot cae al filtro 15m tradicional

    bias = mtf_bias_15m or get_ema_bias(p15)
    if not bias: return None

    # v6.1: actualizar pre-signal state (para alertas tempranas)
    ...
    # v6.1: ¿Es un sweep REAL o micro-sweep?
    ...
    # v7: CONTINUATION ahora BLOQUEA — antes solo penalizaba pero las trampas
    # son la causa principal de señales falsas en Londres/NY open
    if sweep_type == "CONTINUATION":
        details["filtered"] = "⚠ Sweep continuación (trampa) bloqueado"
        return None
    elif sweep_type == "REVERSA":
        # Reversa = lo que queremos, bonus según confianza
        score += int(sweep_conf * 15)  # v7: subido hasta +15 pts
        details["sweep_quality"] = f"✓ Reversa ({sweep_conf:.0%})"
    elif sweep_type == "NEUTRAL":
        # Neutral: requerir al menos 0.5 de confianza
        if sweep_conf < 0.5:
    ...
    if not bos:
        ...
        if fvg_early and sweep_type == "REVERSA" and sweep_conf >= 0.6:
            ...
            if fvg_lo <= current_price <= fvg_hi:
                ...
                details["score"] = score
                # EARLY entry usa RR más conservador: 1.2:1
                ...
                # Cooldown (v6.3: 60s para que reaccione rápido si setup cambia)
                ...
                return {
                    "direction": direction, "score": score, "msg": msg,
                    "tp": tp, "sl": sl, "rr": rr,
                    "details": details, "session": session,
                }
        return None  # sin BOS ni early entry
    score += 25
    ...
    # ── 5. ML FILTER ─────────────────────────────────────
    # v7: la IA ahora filtra DIRECCIONAL — si contradice al motor, bloquea
    ...
    # ── CANDLE CONFIRMATION (v7: obligatoria para TODAS las señales) ──
    if len(p5) >= 4:
        confirmed = detect_confirmation_candle(p5, bias)
        details["candle"] = "✅ Confirmación 5M" if confirmed else "⏳ Sin confirmación"
        # v7: confirmación obligatoria salvo SNIPER extremo (>95)
        if not confirmed and score < 95:
            details["filtered"] = "sin confirmación de vela 5M"
            return None

    # ── COOLDOWN (v6.3: reducido a 60s para que la señal cambie rápido si el setup falla) ──
...
# Tracking de las últimas N señales reales (ganó/perdió) para detectar degradación
_signal_history = []  # lista de dicts: {ts, direction, won, pnl_r, score}
_performance_monitor = {
    "rolling_wr":       0.0,   # win rate de últimas 50 señales
    "rolling_pf":       1.0,   # profit factor
    "rolling_sharpe":   0.0,   # sharpe ratio anualizado
    "status":           "OK",  # OK | WARNING | DEGRADED | SHUTDOWN
    "baseline_wr":      55.0,  # WR esperado según backtest
    "baseline_pf":      1.5,   # PF esperado según backtest
    "baseline_sharpe":  1.0,   # Sharpe esperado
    "last_update":      0,
    "shutdown_until":   0,     # timestamp hasta cuando está pausado
    "shutdown_reason":  "",
}
PERF_WINDOW = 50   # trades para calcular rolling metrics
PERF_MIN_N  = 20   # mínimo de trades antes de evaluar

def update_performance_metrics():
    """v7: recalcula rolling win rate, profit factor y Sharpe de las últimas señales."""
    ...
    if wr < b_wr * 0.8 or sharpe < b_sharpe * 0.5:
        ...
        if last10_wr < 30 and n >= 30:
            ...
            _performance_monitor["shutdown_until"]  = time.time() + 3600  # 1 hora pausa
...
def is_performance_ok():
    """Retorna (ok, reason). Si está en shutdown, bloquea nuevas señales."""
...
def check_risk_limits():
    ...
    # v7: verificar rolling performance primero
...
def register_signal_result(won, direction="", pnl_r=0.0, score=0):
    """v7: registra resultado y actualiza rolling performance monitor."""
    ...
    # v7: agregar al rolling history
    _signal_history.append({
        "ts": time.time(),
        "direction": direction,
        "won": won,
        "pnl_r": pnl_r if pnl_r else (1.5 if won else -1.0),  # default 1:1.5 RR
        "score": score,
    })
    # Mantener solo últimas 200 señales
    if len(_signal_history) > 200:
        del _signal_history[:-200]
    # Recalcular métricas rolling
...
TWELVE_DAILY_LIMIT = 750   # dejar margen vs 800
