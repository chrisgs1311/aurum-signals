...
def get_historical_ohlc(interval="5min", outputsize=150):
    ...
    try:
        ...
        print(f"  ✓ {len(candles)} velas {interval} cargadas (última: {candles[-1]['close']:.2f})")
        return candles
    except Exception as e:
...
async function fetchAndAnalyze(){
  ...
  try{
    // Load real OHLC candles from Twelve Data
    const tf = currentTF || '5min';
    const [ohlcRes, priceRes] = await Promise.all([
      fetch('/ohlc?interval='+tf+'&size=200'),
      fetch('/precio')
    ]);
    const candles = await ohlcRes.json();
    const priceData = await priceRes.json();

    if(Array.isArray(candles) && candles.length > 0){
      // Use real OHLC close prices
      ohlcData = candles;
      ...
      if(prices.length===0){let p=priceData.price;const arr=[];for(let i=150;i>0;i--){p=Math.max(priceData.price*.97,p+(Math.random()-.5)*4);arr.unshift(p)}arr[arr.length-1]=priceData.price;prices=arr;}
      else{prices.push(priceData.price);if(prices.length>300)prices=prices.slice(-300)}
      document.getElementById('sourceBadge').innerHTML='<span class="source-badge live">● PRECIO REAL</span>';
    } else throw new Error('sin datos');

    // Current price display
    const livePrice = priceData.price || prices[prices.length-1];
  ...
  }catch(e){btn.disabled=false;btn.querySelector('span').textContent='↻ ACTUALIZAR';document.getElementById('timestamp').textContent='⚠ Error al obtener datos';return}
