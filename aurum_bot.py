...
def _worker_price():
    """Worker #1: precio cada 3s — balanceado entre velocidad y rate limits.
    gold-api.com free ≈ 60 req/min. Con 3s = 20 req/min (seguro)."""
    while True:
        try:
            d = _fetch_with_retry("https://api.gold-api.com/price/XAU",
                                  timeout=5, retries=1, backoff=0.5)
            if d and "price" in d:
                result = {"price": float(d["price"]),
                          "ch": float(d.get("ch", 0)),
...
def _worker_ohlc():
    """Worker #2: OHLC cada 60s — Twelve Data free = 8 calls/min."""
    while True:
        try:
            for interval, tf, size in [("5min", "5m", 200), ("15min", "15m", 100)]:
                ...
                time.sleep(5)  # pausa entre calls para no saturar API
        except Exception as e:
        ...
        try:
            ...
            if cp:
                ...
                if result:
                    ...
                    # Telegram automático para SNIPER
                    if result["score"] >= SCORE_SNIPER and TELEGRAM_TOKEN and TELEGRAM_CHAT_ID:
...
class Handler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # v6.1: /precio — lectura instantánea de cache RAM (0ms)
        elif path == "/precio":
            c = _live_cache["price"]
        ...
        # v6.1: /ohlc — lectura de cache, fallback a API
            ...
            tf_key = "ohlc_5m" if "5" in interval else "ohlc_15m"
            cached_candles = _live_cache.get(tf_key, [])
            if cached_candles:
        ...
        # v6.1: /aipredict — lectura de cache RAM
        elif path == "/aipredict":
            self._send(200, "application/json", json.dumps({
                "prob": _live_cache["ai_prob"],
                "signal": _live_cache["ai_signal"], "trained": _ai.trained,
...
def main():
    ...
    print("  LOW LATENCY: SSE + 500ms price + 2s signal + OHLC real")
    print("=" * 54)
