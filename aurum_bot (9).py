"""
AURUM - Bot de Señales XAU/USD v5.0
...
# ── [FIX 2] HISTORIAL GLOBAL DE PRECIOS ──────────────────
price_history = []
def push_price(p):
...
class AurumAI:
    ...
    def extract_features(self, prices):
        ...
        return [rsi, ema_cross, macd_norm, atr_norm, pvs, mom5, mom10, max(0, min(2, vol))]
    ...
    while True:
        ...
        try:
            ...
            if len(price_history) >= 60:
                ai_train_if_needed(price_history)
        except Exception as e:
            print(f"  ⚠ BG worker: {e}")
...
# Reemplaza el motor ICT original.
# Lógica: Sweep + BOS + FVG + EMA filter + ML filter
# ML decide SOLO si bloquear — no decide la señal.
...
def _swing_highs(prices, n=3):
    """Puntos de swing high"""
    highs = []
    for i in range(n, len(prices)-n):
        if all(prices[i] >= prices[i-j] for j in range(1,n+1)) and            all(prices[i] >= prices[i+j] for j in range(1,n+1)):
...
def detect_sweep_and_rejection(prices, bias):
    """
    Sweep + rechazo — lo más importante del sistema.
    bias='bullish': sweep de lows (stop hunt abajo → long)
    bias='bearish': sweep de highs (stop hunt arriba → short)
    Retorna (swept, level, rejection_strength)
    """
    if len(prices) < 20: return False, 0, 0
    p = prices

    if bias == "bullish":
        # Buscar el low previo significativo
        lows = _swing_lows(p[:-3], n=2)
        if not lows: return False, 0, 0
        prev_low_idx, prev_low = lows[-1]
        # Verificar que el precio perforó ese low recientemente
        recent_low = min(p[-5:])
        recent_close = p[-1]
        if recent_low < prev_low and recent_close > prev_low:
            # Rechazo: medir wick inferior
...
def detect_bos_scalp(prices, bias):
    """
    Break of Structure — confirma la dirección.
    Busca ruptura del último swing en dirección del bias.
    """
    if len(prices) < 15: return False, "none"
    p = prices

    if bias == "bullish":
        # BOS: precio rompe último swing high relevante
        highs = _swing_highs(p[:-2], n=2)
        if highs and p[-1] > highs[-1][1]:
            return True, "BOS alcista"
        # CHOCH: primer HL tras tendencia bajista
...
def detect_fvg_scalp(prices, bias):
    """
    Fair Value Gap — zona de entrada óptima.
    3 velas: gap entre vela 1 y vela 3.
    """
...
def get_ema_bias(prices):
    """
    EMA 9/21 — solo filtro de tendencia.
...
def detect_chop(prices, n=20):
    """Detecta mercado lateral — no operar."""
    ...
    # Lateral si el rango es menor a 5x ATR
    if rng < atr * 5: return True
    # Sin BOS en las últimas n velas
...
def detect_confirmation_candle(prices, bias):
    """
    Vela de confirmación 1M — entry trigger.
    Busca vela de impulso en dirección del bias.
    """
...
def get_ml_filter(prices):
    """
    ML como filtro — NO decide la señal.
    HIGH (≥0.7) → ok
    MED (0.5–0.7) → ok con filtro adicional
    LOW (<0.5) → bloquear
    """
    if not _ai.trained or len(prices) < 35:
        return "MED", 0.5   # sin modelo = permisivo
    prob = _ai.predict_proba(prices)
...
def update_ict_prices(price):
    ...
    global _scalp_prices_1m, _scalp_prices_5m, _scalp_prices_15m
    _scalp_prices_1m.append(price)
    if len(_scalp_prices_1m) > 500: _scalp_prices_1m = _scalp_prices_1m[-500:]
    # Cada 5 precios 1M → 1 precio 5M (aproximado)
    if len(_scalp_prices_1m) % 5 == 0:
        _scalp_prices_5m.append(price)
        if len(_scalp_prices_5m) > 300: _scalp_prices_5m = _scalp_prices_5m[-300:]
    # Cada 15 precios 1M → 1 precio 15M
    if len(_scalp_prices_1m) % 15 == 0:
        _scalp_prices_15m.append(price)
        if len(_scalp_prices_15m) > 200: _scalp_prices_15m = _scalp_prices_15m[-200:]
...
def run_ict_engine(current_price, atr):
    """
    AURUM SCALP v2 — Motor principal.
    ...
    # ── 1. LIQUIDITY SWEEP (40pts) — OBLIGATORIO ──────────
    swept, sweep_level, rejection = detect_sweep_and_rejection(p5, bias)
    if not swept: return None   # sin sweep = no trade
    score += 40
    details["sweep"] = f"Sweep ${sweep_level} | rej={rejection:.1f}"

    # ── 2. BOS / CHOCH (25pts) — OBLIGATORIO ─────────────
    bos, bos_type = detect_bos_scalp(p5, bias)
    if not bos: return None    # sin BOS = no confirma dirección
    score += 25
    details["bos"] = bos_type

    # ── 3. FVG (15pts) ────────────────────────────────────
    fvg, fvg_lo, fvg_hi = detect_fvg_scalp(p5, bias)
    if fvg:
        score += 15
        details["fvg"] = f"FVG ${fvg_lo}–${fvg_hi}"
        # Bonus: precio dentro del FVG
    ...
    # ── RSI — SOFT FILTER (penaliza score, NO bloquea) ──
    # RSI nunca cancela un trade — solo ajusta score.
    # La estructura (Sweep+BOS) manda. RSI solo filtra extremos.
    rsi = get_rsi_scalp(p5)
    details["rsi"] = round(rsi, 1)
    if bias == "bullish":
        if   rsi > 75: score -= 15   # sobrecomprado — reduce score
        elif rsi > 65: score -= 5    # moderadamente alto
        elif rsi < 40: score += 5    # buen nivel para long
    if bias == "bearish":
        if   rsi < 25: score -= 15   # sobrevendido — reduce score
        elif rsi < 35: score -= 5    # moderadamente bajo
        elif rsi > 60: score += 5    # buen nivel para short
    # Solo si RSI está EXTREMO y score cae bajo el umbral → no trade
    if score < SCORE_NORMAL: return None

    # ── CANDLE CONFIRMATION (1M) ─────────────────────────
    if len(p1) >= 4:
        ...
        if not confirmed and score < SCORE_SNIPER:
            return None  # esperamos confirmación si no es sniper
    ...
    # RR dinámico: sniper=2.5R, normal=1.5R
...
<body>
<header>
  <div class="logo">AURUM<span>BOT DE SEÑALES XAU/USD v5.0</span></div>
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-top:20px">
    ...
    <div>
      ...
      <div class="rules-box">
        ...
        <div class="rule-row" style="border:none"><span class="rule-label">VERSIÓN</span><span style="color:var(--dim2);font-size:10px">AURUM v5.0</span></div>
      </div>
    </div>
  </div>
</div>
...
function computeSignal(){
  ...
  // Únicos hard blocks en dashboard: confianza mínima + noticias activas
  const cok=confidence>=minConf;checks.push({label:'CONF '+confidence+'%≥'+minConf+'%',ok:cok});if(!cok)valid=false;
  checks.push({label:'1H '+htfTrend.toUpperCase(),ok:mtfOk}); // contexto, no bloquea
  if(newsBlocked){checks.push({label:'NOTICIA ACTIVA',ok:false});valid=false;}
  // RSI / MACD / MEDIAS = contexto visual únicamente
  // NUNCA bloquean. Solo el score decide la entrada.
  // Bloqueos duros: sin Sweep, sin BOS, ML LOW, chop, fuera de sesión.
...
async function fetchAndAnalyze(){
  ...
  const d=computeSignal();currentSignalData=d;triggerSound(d);
    ...
    def do_GET(self):
        ...
        if path == "/":
            ...
            self._send(200, "application/json", json.dumps({
                ...
                "trending":         is_trending_market(_ict_prices_5m) if _ict_prices_5m else False,
            ...
            # Run full backtest on real historical data
...
def main():
    init_log()
    print("=" * 54)
    print("  AURUM v5.0 · Bot de Señales XAU/USD")
    print("  Fixes: cache ✓ | price_history ✓ | BG worker ✓ | ICT ✓")
    print("=" * 54)
