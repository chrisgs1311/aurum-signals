...
# v6.2: URLs de precio — Twelve Data primero (mismo feed que TradingView Forex.com)
_PRICE_APIS = [
    # Twelve Data REST /price → mismo feed que TradingView/Forex.com
    ...
    # Fallback: gold-api.com (spot LBMA, puede diferir $2-5 de Forex.com)
...
def main():
    ...
    print("  LOW LATENCY: SSE + 3s price + 2s signal + OHLC real")
    print("=" * 54)
    ...
    # v6.2: pre-train con velas OHLC REALES (5m + 15m), no solo price_history
    print("\n  Pre-entrenando IA con velas reales...")
    with _data_lock:
        ohlc5_closes  = [c["c"] for c in _ohlc_candles_5m]
        ohlc15_closes = [c["c"] for c in _ohlc_candles_15m]
    train_data = ohlc15_closes + ohlc5_closes  # 15m primero, 5m al final
    if len(train_data) < 60:
        train_data = list(price_history)  # fallback
    if len(train_data) >= 60:
    ...
    # v6.1: lanzar workers de baja latencia
    ...
    print(f"\n  ⚡ Latencia: precio 3s | señal 2s | SSE push instantáneo")
    print("\n  Para detener: Ctrl + C")
    print("-" * 54)
