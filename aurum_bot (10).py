...
HIGH_IMPACT_EVENTS = [
    # MARTES
    (1, 13, 30, "CPI USA", "CRITICAL"),
    (1, 15,  0, "JOLTS Job Openings", "HIGH"),
    # MIÉRCOLES
    (2, 13, 15, "ADP Employment", "HIGH"),
    (2, 14,  0, "FOMC Minutes", "CRITICAL"),
    (2, 15,  0, "ISM Services PMI", "HIGH"),
    (2, 19,  0, "FOMC Rate Decision", "CRITICAL"),
    # JUEVES
    (3, 12, 15, "ECB Rate Decision", "HIGH"),
    (3, 13, 30, "PPI USA", "HIGH"),
    (3, 13, 30, "Retail Sales", "HIGH"),
    (3, 13, 30, "GDP USA", "HIGH"),
    # VIERNES
    (4, 13, 30, "NFP / Non-Farm Payrolls", "CRITICAL"),
    (4, 13, 30, "Unemployment Rate", "CRITICAL"),
    (4, 13, 30, "Core PCE Price Index", "CRITICAL"),
]
PAUSE_MINUTES_BEFORE = 30
PAUSE_MINUTES_AFTER  = 30
ALERT_MINUTES_BEFORE = [60, 15, 5]  # alertas a 60min, 15min y 5min antes
...
def is_news_time():
    ...
    for ev in HIGH_IMPACT_EVENTS:
        day, hour, minute, name = ev[0], ev[1], ev[2], ev[3]
        if dow != day: continue
...
def check_upcoming_news():
    """v6.1: Detecta noticias próximas y devuelve lista de alertas a enviar."""
    now = _dt.datetime.utcnow()
    dow = now.weekday()
    alerts = []
    for ev in HIGH_IMPACT_EVENTS:
        day, hour, minute, name = ev[0], ev[1], ev[2], ev[3]
        impact = ev[4] if len(ev) > 4 else "HIGH"
        if dow != day: continue
        event_time = now.replace(hour=hour, minute=minute, second=0, microsecond=0)
        mins_until = (event_time - now).total_seconds() / 60
        # Verificar si estamos en una ventana de alerta
        for alert_min in ALERT_MINUTES_BEFORE:
            if abs(mins_until - alert_min) < 0.6:  # ventana de ~1min
                alert_key = f"{name}_{event_time.strftime('%Y%m%d%H%M')}_{alert_min}"
                if alert_key not in _news_alerted:
                    _news_alerted.add(alert_key)
                    alerts.append({
                        "name": name,
                        "impact": impact,
                        "mins_until": int(alert_min),
                        "event_time_utc": event_time.strftime("%H:%M UTC"),
                    })
    # Limpiar tracking viejo (más de 1 día)
...
# v6.1: URLs de precio con fallback
_PRICE_APIS = [
    ("https://api.gold-api.com/price/XAU", lambda d: {"price": float(d["price"]), "ch": float(d.get("ch",0)), "chp": float(d.get("chp",0))} if "price" in d else None),
    ("https://api.metalpriceapi.com/v1/latest?api_key=demo&base=XAU&currencies=USD", lambda d: {"price": 1.0/float(d["rates"]["USD"]), "ch": 0, "chp": 0} if d.get("rates",{}).get("USD") else None),
]
...
def _worker_price():
    ...
    while True:
        try:
            ...
            sleep_time = 10 if ws_active else 1
...
def _worker_signal():
    """Worker #3: señal SCALP v2 cada 2s — detecta y pushea instantáneo."""
    while True:
        try:
            atr_val = 0.0
            with _data_lock:
            ...
            if len(p5) >= 15:
                atr_val = sum(abs(p5[i]-p5[i-1])
                              for i in range(len(p5)-14, len(p5))) / 14
            cp = p5[-1] if p5 else 0
            if cp:
                result = run_ict_engine(cp, atr_val)
                # v6.1: push pre-signal state (siempre, incluso sin señal)
                _push_sse("presignal", {
                ...
                if result:
                    ...
                    # v6.1: Telegram para TODAS las señales válidas (SNIPER + NORMAL)
        ...
        time.sleep(1)  # v6.2: 1s (antes 2s) para detectar señales más rápido
...
def _worker_news():
    ...
    Chequea cada 60s y envía aviso 60/15/5 min antes de cada evento."""
        ...
        try:
            ...
            for a in alerts:
                ...
                msg = (
                    ...
                    f"📌 Evento: *{a['name']}*\n"
                    f"⏰ Hora: {a['event_time_utc']}\n"
                    f"⏳ Faltan: *{a['mins_until']} min*\n"
                    f"📊 Impacto: {a['impact']}\n\n"
                    f"━━━━━━━━━━━━━━━━━━\n"
                ...
                print(f"  📢 Alerta noticia enviada: {a['name']} ({a['mins_until']}min)")
        except Exception as e:
...
def run_ict_engine(current_price, atr):
    ...
    v6.2: usa closes de velas OHLC REALES (5min de Twelve Data) como fuente primaria.
    El buffer sintético solo se usa como fallback si no hay OHLC.
    Orden: Sweep(real) → BOS → FVG → EMA → ML → Score
    """
    global _last_scalp_signal
    import time as _t

    # v6.2: FUENTE PRIMARIA = velas OHLC reales de Twelve Data
    # Antes usaba _scalp_prices_5m (buffer sintético que se llenaba cada 5 TICKS, no 5 min)
    # Ahora usa los closes de las velas REALES de 5 minutos
    ohlc5 = _ohlc_candles_5m if len(_ohlc_candles_5m) >= 20 else None
    ohlc15 = _ohlc_candles_15m if len(_ohlc_candles_15m) >= 15 else None

    if ohlc5:
        # Closes de velas 5m REALES
        p5 = [c["c"] for c in ohlc5]
        # Agregar el precio live actual como "vela en formación"
        if current_price and abs(current_price - p5[-1]) > 0.01:
            p5 = p5 + [current_price]
    else:
        # Fallback: buffer sintético
        p5 = _scalp_prices_5m if len(_scalp_prices_5m) >= 20 else _scalp_prices_1m

    if ohlc15:
        p15 = [c["c"] for c in ohlc15]
    elif len(_scalp_prices_15m) >= 15:
        p15 = _scalp_prices_15m
    else:
        p15 = p5

    # p1 sigue siendo el buffer tick (para confirmación de vela micro)
    p1 = _scalp_prices_1m
  ...
  evtSource.addEventListener('signal',function(e){
    try{
      ...
      if(s&&s.direction){
        ...
        if(prices.length)document.getElementById('lvlEntry').textContent='$'+prices[prices.length-1].toFixed(2);
        currentSignalData={valid:true,rawSignal:s.direction,signal:s.direction,cls:cls,confidence:s.score,price:prices[prices.length-1]||0,tpDist:Math.abs(s.tp-(prices[prices.length-1]||0)),slDist:Math.abs(s.sl-(prices[prices.length-1]||0)),rr:s.rr,ATR:0,checks:[],reason:'SCALP v2 SSE'};
