...
# v6.3: API key DEBE estar definida antes de _PRICE_APIS
TWELVE_API_KEY = os.environ.get("TWELVE_API_KEY", "dd53883de1a84cccaf65bf7f4e7a4756")

# v6.3: URLs de precio — gold-api PRIMERO (confiable, sin límites), Twelve Data como backup
# Razón: gold-api.com no requiere key y nunca falla. Si Twelve Data fue revocada
# o excedió el límite diario, el bot NO debe colgarse — debe caer a gold-api automático.
_PRICE_APIS = [
    # Primario: gold-api.com (spot LBMA, 100% confiable, sin API key)
    ("https://api.gold-api.com/price/XAU",
     lambda d: {"price": float(d["price"]), "ch": float(d.get("ch",0)), "chp": float(d.get("chp",0))} if "price" in d else None),
    # Backup: Twelve Data REST (mismo feed que TradingView pero usa key del plan Free)
...
def _worker_price():
    ...
    while True:
        try:
            ...
            # v6.2: sleeps ajustados para no quemar Twelve Data
            # WS activo → HTTP solo valida cada 30s (2880 calls/día)
            # WS caído → HTTP toma control cada 5s (17k calls/día, supera límite)
            # Si supera límite → rota a gold-api automáticamente via _PRICE_APIS
            ...
            if "twelvedata.com" in url and not _can_call_twelve():
                # Rotar a gold-api si excedimos Twelve Data
                api_idx = 1
                url, parser = _PRICE_APIS[api_idx]
            ...
            if result and result["price"] > 0:
                ...
                if not ws_active or ws_age > 5:
                    _live_cache["price"] = result
                    _live_cache["price_ts"] = time.time()
                    _live_cache["price_fails"] = 0
                    # v6.2 FIX: solo push_price (ML history), NO update_ict_prices
...
def _worker_websocket():
    ...
    v6.3: se deshabilita permanentemente si la key es inválida."""
    while True:
        ...
        try:
            ...
            print("  ✅ WebSocket conectado, suscrito a XAU/USD")
            ...
            while True:
                try:
                    ...
                    if not msg: continue
                    data = json.loads(msg)
                    # Twelve Data envía: {"event":"price","symbol":"XAU/USD","price":4732.5,...}
                    if data.get("event") == "price" and "price" in data:
                        ...
                        if price > 0:
                            ...
                            # v6.2 FIX: WebSocket SOLO actualiza display + price_history (ML)
                            # NO llamar update_ict_prices() — alimentaría velas falsas al motor SCALP
                            # Los buffers _scalp_prices_* solo se llenan con OHLC real del worker
                            push_price(price)
                            _push_sse("price", result)
                    ...
                    # Enviar heartbeat para mantener viva
                    _ws_send_frame(sock, json.dumps({"action": "heartbeat"}))
                    last_heartbeat = time.time()
                    continue
        except Exception as e:
            print(f"  ⚠ WebSocket: {e} — reintentando en 10s")
            _live_cache["ws_active"] = False
            try: sock.close()
            except: pass
            time.sleep(10)
...
def _worker_ohlc():
    ...
    intervals = {
        "5min":  {"tf": "5m",  "size": 200, "freq": 180},   # cada 3min
        "15min": {"tf": "15m", "size": 100, "freq": 300},   # cada 5min
        "1h":    {"tf": "1h",  "size": 50,  "freq": 600},   # cada 10min
        "4h":    {"tf": "4h",  "size": 200, "freq": 1800},  # cada 30min
    }
    while True:
        try:
            now = time.time()
...
def _worker_signal():
    ...
    v6.3: BLOQUEA señales con datos viejos. Previene entradas falsas.
    Validaciones críticas antes de generar señal:
    1. Precio fresco (< 15s de antigüedad)
    2. OHLC 5m fresco (< 5 min)
    3. WebSocket activo o HTTP reciente
    4. Datos OHLC suficientes (20+ velas 5m, 15+ velas 15m)
    """
    while True:
        try:
            now_ts = time.time()
            
            # ── VALIDACIÓN 1: edad del precio ──
            price_ts = _live_cache.get("price_ts", 0)
            price_age = now_ts - price_ts if price_ts else 999
            if price_age > 15:  # más de 15s sin precio = STALE
                _push_sse("wait", {
                    "reason": f"⚠ Precio stale ({int(price_age)}s) — esperando datos frescos",
                    "blocked": True,
                })
            ...
            # ── VALIDACIÓN 2: edad de OHLC ──
            ohlc_ts = _live_cache.get("ohlc_ts", 0)
            ohlc_age = now_ts - ohlc_ts if ohlc_ts else 999
            if ohlc_age > 360:  # más de 6 min = STALE
                _push_sse("wait", {
                    "reason": f"⚠ OHLC stale ({int(ohlc_age/60)}min) — esperando velas frescas",
                    "blocked": True,
                })
                _live_cache["signal"] = None
                time.sleep(1)
            ...
            # ── VALIDACIÓN 3: cantidad de datos reales ──
            with _data_lock:
            ...
            if ohlc5_count < 20 or ohlc15_count < 15:
                _push_sse("wait", {
                    "reason": f"⚠ Cargando velas ({ohlc5_count}/20 5m, {ohlc15_count}/15 15m)",
                    "blocked": True,
                })
                _live_cache["signal"] = None
                time.sleep(1)
            ...
            # ── VALIDACIÓN 5: precio coherente con última vela 5m ──
            # Si el precio live difiere >2% de la última vela 5m, algo está mal
            last_close = ohlc5_local[-1]["c"]
            price_diff_pct = abs(cp - last_close) / last_close * 100
            if price_diff_pct > 2.0:
                _push_sse("wait", {
                    "reason": f"⚠ Precio incoherente: live ${cp:.2f} vs vela ${last_close:.2f} ({price_diff_pct:.1f}%)",
                    "blocked": True,
                })
                _live_cache["signal"] = None
                time.sleep(1)
            ...
            if result:
                # ── VALIDACIÓN 6 (post-motor): TP/SL coherentes ──
                tp_diff = abs(result["tp"] - cp)
                sl_diff = abs(result["sl"] - cp)
                # TP y SL deben estar al menos 0.05% del precio (~$2 en oro a $4000)
                min_diff = cp * 0.0005
                if tp_diff < min_diff or sl_diff < min_diff:
                    _push_sse("wait", {
                        "reason": f"⚠ TP/SL muy cerca del precio (${tp_diff:.2f}/${sl_diff:.2f})",
                        "blocked": True,
                    })
                    _live_cache["signal"] = None
                    time.sleep(1)
                ...
                # ── VALIDACIÓN 7: dirección coherente con TP/SL ──
                is_buy = result["direction"] == "COMPRAR"
                if is_buy and (result["tp"] <= cp or result["sl"] >= cp):
                    print(f"  ⚠ Señal BUY inválida: TP={result['tp']} SL={result['sl']} cp={cp}")
                    _live_cache["signal"] = None
                    time.sleep(1)
                    continue
                if not is_buy and (result["tp"] >= cp or result["sl"] <= cp):
                    print(f"  ⚠ Señal SELL inválida: TP={result['tp']} SL={result['sl']} cp={cp}")
                    _live_cache["signal"] = None
                    time.sleep(1)
                    continue
                ...
                # No hay setup válido → publicar WAIT instantáneo
                last_sig_age = now_ts - _live_cache.get("signal_ts", 0)
                if last_sig_age > 5:
                    if _live_cache.get("signal") is not None:
                        _live_cache["signal"] = None
                        _push_sse("wait", {
                            "reason": "Sin setup válido",
                            "presignal_state": _pre_signal["state"],
                            "blocked": False,
                        })
        except Exception as e:
    ...
    while True:
        ...
        try:
            ...
            if len(ph_full) >= 60:
                ...
                # Actualizar cache de AI
                prob, sig = ai_predict(ph_full)
                _live_cache["ai_prob"] = round(prob * 100, 1) if prob is not None else None
                _live_cache["ai_signal"] = sig
                print(f"  🤖 AI cache updated: prob={_live_cache['ai_prob']} sig={sig}")
            else:
                print(f"  ⏳ AI: esperando datos ({len(ph_full)}/60 precios)")
        except Exception as e:
...
def start_all_workers():
    ...
    workers = [
        ("WebSocket Live (~1s)", _worker_websocket),
        ("Price HTTP (fallback)", _worker_price),
        ("OHLC Feed (3min)",     _worker_ohlc),
...
SCORE_EARLY  = 60   # v6.1: entrada temprana en FVG (pre-signal)
...
def run_ict_engine(current_price, atr):
    ...
    if sweep_type == "CONTINUATION":
        # Continuación = trampa, penalizar fuerte
        score -= 20
        details["sweep_warning"] = "⚠ Posible trampa (continuación)"
    elif sweep_type == "REVERSA":
        # Reversa = lo que queremos, bonus
        score += int(sweep_conf * 10)  # hasta +10 pts por confianza
    ...
    # ── CANDLE CONFIRMATION (v6.3: usa últimas velas 5m REALES) ──
    if len(p5) >= 4:
...
def get_gold_price():
    ...
    if c: return c
    d = _fetch_with_retry("https://api.gold-api.com/price/XAU", timeout=5, retries=2, backoff=0.5)
    if d and "price" in d:
...
def get_historical_ohlc(interval="5min", outputsize=150):
    ...
    if c: return c
    # v6.3: si la key fue marcada como inválida, no volver a intentar
    if _live_cache.get("twelve_key_invalid"):
        return []
    # v6.1: verificar límite diario antes de llamar
    if not _can_call_twelve():
    ...
    if data and "values" in data:
        ...
        print(f"  ✓ {len(candles)} velas {interval} [API call #{_api_counter['twelve_calls']}/día]")
        return candles
    elif data:
        msg = data.get('message','sin datos')
        print(f"  ⚠ Twelve Data: {msg}")
        # v6.3: detectar errores de key inválida → deshabilitar Twelve Data permanentemente
        if any(kw in str(msg).lower() for kw in ["api key", "apikey", "invalid", "unauthor", "forbidden", "not registered"]):
            print(f"  ❌ TWELVE DATA API KEY INVÁLIDA — cambiando a gold-api.com permanentemente")
            print(f"  💡 Fix: actualiza TWELVE_API_KEY en Render environment variables")
...
// v6.1: SSE — señales push instantáneas
let evtSource=null;
// v6.2: interpolación de precio para movimiento fluido tipo TradingView
let _targetPrice=null,_currentPrice=null,_interpolating=false;
function _animatePrice(){
  if(_currentPrice===null||_targetPrice===null){_interpolating=false;return;}
  const diff=_targetPrice-_currentPrice;
  if(Math.abs(diff)<0.001){
    _currentPrice=_targetPrice;
    document.getElementById('priceDisplay').textContent='$'+_currentPrice.toFixed(2);
    _interpolating=false;
    return;
  }
  // Easing: cada frame avanza ~15% hacia el target (smooth)
  _currentPrice+=diff*0.15;
  document.getElementById('priceDisplay').textContent='$'+_currentPrice.toFixed(2);
  requestAnimationFrame(_animatePrice);
}
function setLivePrice(newPrice){
  _targetPrice=newPrice;
  if(_currentPrice===null)_currentPrice=newPrice;
  if(!_interpolating){
    _interpolating=true;
    requestAnimationFrame(_animatePrice);
  }
...
function initSSE(){
  ...
  evtSource.addEventListener('price',function(e){
    try{
      ...
      if(pd&&pd.price){
        // v6.2: usar interpolación animada en vez de cambio brusco
        setLivePrice(pd.price);
...
class Handler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # v6.1: /aipredict — cache RAM con fallback a cálculo en vivo
            ...
            # Si cache vacío, calcular en vivo (no ideal pero mejor que null)
            if prob_val is None and _ai.trained:
                with _data_lock:
                    ph = list(price_history)
                if len(ph) >= 35:
                    prob_raw, sig_raw = ai_predict(ph)
...
def _background_init():
    ...
    for interval, tf, size in [("5min", "5m", 50), ("15min", "15m", 30), ("1h", "1h", 24), ("4h", "4h", 200)]:
        candles = get_historical_ohlc(interval, size)
        if candles:
