...
def _worker_websocket_massive():
    ...
    while True:
        try:
            ...
            # Subscribe: Quotes (C) + Second Aggregates (CAS) para XAUUSD
            # C.C:XAUUSD = Quotes (bid/ask real-time)
            # CAS.C:XAUUSD = Second Aggregates (OHLC por segundo)
            sub_msg = json.dumps({"action": "subscribe", "params": "C.C:XAUUSD,CAS.C:XAUUSD"})
            _ws_send_frame(sock, sub_msg)
            print("  ✅ Subscribed C:XAUUSD (quotes + per-second bars)")
            ...
            while True:
                try:
                    ...
                    for ev in events:
                        ev_type = ev.get("ev", "")
                        # Quote: {"ev":"C","p":"XAUUSD","x":48,"a":4793.25,"b":4793.15,"t":...}
                        if ev_type == "C":
                            bid = ev.get("b")
                            ask = ev.get("a")
                            if bid and ask:
                                ...
                                last_price = price
                        # Second Aggregate: {"ev":"CAS","pair":"XAU/USD","o":..,"c":..,"h":..,"l":..,"s":timestamp}
                        elif ev_type == "CAS":
                            close = ev.get("c")
                            if close:
                                price = float(close)
