...
(2, 14, 0,  “FOMC Minutes”),  # Miércoles 14:00 GMT aprox(4, 13, 30, “NFP / Jobs”),    # Viernes 13:30 GMT aprox
...
score = 1.0
    ...
    h = _mdt.datetime.utcnow().hour
    if 8 <= h < 17:    session_factor = 1.0   # Overlap perfecto
    elif 7 <= h < 8:   session_factor = 0.7   # Pre-Londres
    elif 17 <= h < 22: session_factor = 0.7   # NY tarde
    else:              session_factor = 0.2   # Fuera de sesión
    ...
    if len(prices) >= 15:
        ...
        if atr_pct < 0.05:  vol_factor = 0.3   # mercado muerto
        elif atr_pct < 0.1: vol_factor = 0.6   # poca volatilidad
        elif atr_pct < 0.5: vol_factor = 1.0   # volatilidad normal
        elif atr_pct < 1.0: vol_factor = 0.8   # alta volatilidad
        else:               vol_factor = 0.4   # extrema — news spike
...
if decision["state"] != _current_control["state"]:
    if now >= _current_control["lock_until"]:
        ...
        print(f"  🔒 CONTROL LOCK: mantener {_current_control['state']} | {remaining}min restantes")
else:
    ...
    print(f"  ✅ CONTROL: {_current_control['state']} | "
          f"model={m_lvl}({m_sig:.2f}) market={mk_lvl}({mk_sig:.2f}) "
          f"| buy≥{decision['thresholds']['buy']} sell≤{decision['thresholds']['sell']}")
...
def save(self):
    ...
    # Try /data first with atomic write
    for target in [AurumAI.MODEL_FILE, "aurum_model_v2.pkl"]:
        try:
            ...
            print(f"  💾 MODEL SAVED: {target} | acc={self.accuracy:.1f}% | epochs={self.epochs}")
...
def load(self):
    ...
    for path in paths:
        ...
        try:
            ...
            print(f"  ✅ MODEL LOADED FROM: {path} | acc={self.accuracy:.1f}% | epochs={self.epochs}")
            return
        except Exception as e:
...
def save_version(self):
    ...
    try:
        import shutil, glob
        # Copiar modelo actual como versión
        if os.path.exists(AurumAI.MODEL_FILE):
            ...
            # Limpiar versiones antiguas (mantener solo MAX_VERSIONS)
            versions = sorted(glob.glob(f"{AurumAI.MODEL_DIR}/aurum_model_v*.bak"))
...
def rollback(self):
    ...
    try:
        ...
        self.load()  # Recargar desde el backup
...
def save_dataset(self, X, y):
    ...
    try:
        ...
        now_str = _ddt.datetime.utcnow().isoformat()
        # Nuevas muestras tienen peso 1.0 (máximo)
        existing["X"].extend(X)
        existing["y"].extend(y)
        ...
        # Muestras de hace más de 24h pierden 20% de peso
        now = _ddt.datetime.utcnow()
        for i in range(len(existing["timestamps"])):
            try:
                ts = _ddt.datetime.fromisoformat(existing["timestamps"][i])
                hours_old = (now - ts).total_seconds() / 3600
                # Decay: 20% menos por cada 24h de antigüedad
        ...
        if len(existing["X"]) > 2000:
            # Eliminar las más antiguas (menor peso)
        ...
        avg_weight = sum(existing["weights"]) / len(existing["weights"]) if existing["weights"] else 0
        print(f"  📊 DATASET SAVED: {len(existing['X'])} muestras | peso_prom={avg_weight:.2f} | decay_ok=True")
    except Exception as e:
...
STATE_LOCK_SECONDS = 1800   # 30 min máximo — se reduce con lock decay en condiciones extremas
...
def _get_lock_duration(self):
    ...
    """
    # Condición extrema: drift muy alto + accuracy muy baja
    if self.drift_score > 0.8 and self.accuracy < 50:
        print(f"  ⚡ LOCK DECAY: Condición extrema — lock reducido a 5min")
        return 300   # 5 minutos

    # Condición severa: drift alto O accuracy baja
    if self.drift_score > 0.65 or self.accuracy < AurumAI.MIN_ACCURACY:
        print(f"  ⚡ LOCK DECAY: Condición severa — lock reducido a 10min")
        return 600   # 10 minutos

    # Condición moderada: drift medio
    if self.drift_score > 0.4:
        print(f"  ⚡ LOCK DECAY: Condición moderada — lock reducido a 20min")
        return 1200  # 20 minutos

    # Normal — lock completo de 30 min
...
def _can_change_state(self, new_state):
    ...
    import time as _tl
    now = _tl.time()
    # Si el estado no cambia, siempre permitido
    if new_state == AurumAI._current_state:
        return True
    # Si el lock no ha expirado, bloquear cambio
    if now < AurumAI._state_lock_until:
        remaining = int((AurumAI._state_lock_until - now) / 60)
        # Override: si condición es extrema y llevamos más de 5min → permitir
        if self.drift_score > 0.8 and self.accuracy < 50:
            if now - (AurumAI._state_lock_until - AurumAI.STATE_LOCK_SECONDS) > 300:
                print(f"  🚨 LOCK OVERRIDE: Condición extrema — cambio forzado {AurumAI._current_state}→{new_state}")
                return True
        print(f"  🔒 STATE LOCK: No cambiar {AurumAI._current_state}→{new_state} | {remaining}min restantes")
        return False
    return True
...
def _set_state(self, new_state):
    ...
    if new_state != AurumAI._current_state:
        ...
        print(f"  🔄 STATE CHANGE: {AurumAI._current_state} → {new_state} | lock={lock_min}min")
        AurumAI._current_state    = new_state
        AurumAI._state_lock_until = _tl.time() + lock_duration
...
def health_check(self, prev_accuracy=None):
    ...
    if self.trained and self.accuracy < AurumAI.MIN_ACCURACY:
        ...
        print(f"  ⚠ HEALTH: Accuracy baja {self.accuracy:.1f}% | streak={AurumAI._low_acc_streak}/{AurumAI.ROLLBACK_STREAK}")
        if AurumAI._low_acc_streak >= AurumAI.ROLLBACK_STREAK:
            if self._can_change_state("ROLLBACK"):
                ...
                print(f"  🔄 ROLLBACK ejecutado (streak consistente)")
        else:
            if self._can_change_state("CONSERVATIVE"):
                ...
                print(f"  🟡 HEALTH: Modo conservador activado")
    else:
    ...
    if self.is_market_drifting():
        ...
        print(f"  ⚠ HEALTH: Drift {self.drift_score:.2f} | streak={AurumAI._high_drift_streak}/{AurumAI.DRIFT_STREAK}")
        if AurumAI._high_drift_streak >= AurumAI.DRIFT_STREAK:
            if self._can_change_state("DRIFT_CONFIRMED"):
                ...
                print(f"  🔴 HEALTH: Drift confirmado — señales en modo conservador")
        else:
            if self._can_change_state("DRIFT_WARNING"):
                ...
                print(f"  🟡 HEALTH: Drift temprano — modo conservador (no stop)")
    else:
        AurumAI._high_drift_streak = max(0, AurumAI._high_drift_streak - 1)

    # ── Check 3: Caída brusca de accuracy ────────────────
    if prev_accuracy and self.accuracy < prev_accuracy - 15:
        # Caída de más de 15 puntos — activar conservador
        AurumAI.CONSERVATIVE_MODE = True
        report["status"] = "ACCURACY_DROP"
        report["action"] = "CONSERVATIVE_MODE"
        print(f"  ⚠ HEALTH: Accuracy cayó {prev_accuracy:.1f}→{self.accuracy:.1f}% — conservador (no rollback inmediato)")
    elif prev_accuracy and self.accuracy < prev_accuracy - 25:
        # Caída de más de 25 puntos — SÍ rollback
        self.rollback()
        report["action"] = "ROLLBACK_BIG_DROP"
        print(f"  🔄 ROLLBACK: Caída extrema {prev_accuracy:.1f}→{self.accuracy:.1f}%")

    if report["status"] == "OK":
        print(f"  ✅ HEALTH: OK | acc={self.accuracy:.1f}% | drift={self.drift_score:.2f} | conservative={AurumAI.CONSERVATIVE_MODE}")
...
if ctrl_state != "FULL":
    print(f"  🎛 AI [{ctrl_state}]: prob={prob:.2f} | buy≥{buy_thresh} sell≤{sell_thresh} → {signal}")
...
if score >= SCORE_SNIPER: return “SNIPER ENTRY”, “🥇”
if score >= SCORE_SETUP:  return “SETUP”,        “🟡”
return “NO TRADE”, “⚫”
── FILTROS DUROS ─────────────────────────────────────────
...
h = _sdt.datetime.utcnow().hour
if 7 <= h < 8:   return “Pre-Londres 🌅”
if 8 <= h < 13:  return “Londres 🇬🇧”
if 13 <= h < 17: return “Overlap ⚡ NY+LON”
...
“””
if not prices_15m or len(prices_15m) < 12: return None
p = prices_15m
block = 3
highs = [max(p[i:i+block]) for i in range(0, len(p)-block, block)]
lows  = [min(p[i:i+block]) for i in range(0, len(p)-block, block)]
...
if last_move > p[-1] * 0.003: return “bullish”
if last_move < -p[-1] * 0.003: return “bearish”
return None
── LIQUIDEZ — CORE DEL SISTEMA ───────────────────────────
def detect_equal_highs(prices, tolerance=0.0008):
“”“Equal Highs — BSL acumulada arriba”””
if len(prices) < 10: return False, 0
highs = []
for i in range(1, len(prices)-1):
if prices[i] >= prices[i-1] and prices[i] >= prices[i+1]:
highs.append(prices[i])
if len(highs) < 2: return False, 0
...
if bias == "bullish":
    ...
    # Sweep: rompió el low y volvió arriba (rechazo)
...
if bias == "bearish":
    ...
    current_price = prices[-1]
    # Sweep: rompió el high y volvió abajo (rechazo)
...
gap_size = high3 - low1
if gap_size > prices[-1] * 0.001:  # FVG significativo
...
“””
if len(prices) < 10: return False, 0, 0
p = prices

if bias == "bullish":
    # Buscar última vela bajista antes de impulso alcista
    for i in range(len(p)-5, max(0, len(p)-15), -1):
        if p[i] < p[i-1]:  # vela bajista
            ...
            if i+3 < len(p) and p[i+3] > p[i] * 1.002:
                ...
                # OB válido si precio no lo ha vuelto a tocar
                if min(p[i+1:]) > ob_low:
                    return True, ob_low, ob_high

if bias == "bearish":
    # Buscar última vela alcista antes de impulso bajista
    for i in range(len(p)-5, max(0, len(p)-15), -1):
...
if len(prices) < 27: return 0
def ema(arr, n):
k=2/(n+1); e=sum(arr[:n])/n
for v in arr[n:]: e=vk+e(1-k)
return e
...
if len(_ict_prices_5m) > 500: _ict_prices_5m = _ict_prices_5m[-500:]
if len(_ict_prices_5m) % 3 == 0:
_ict_prices_15m.append(price)
if len(_ict_prices_15m) > 200: _ict_prices_15m = _ict_prices_15m[-200:]
...
if len(p5) < 30: return None

# ── FILTRO 1: SESIÓN ──────────────────────────────────
if not is_trading_session(): return None

# ── FILTRO 2: TRENDING MARKET ─────────────────────────
if not is_trending_market(p5): return None
...
if fvg: details["fvg"] = f"FVG ${fvg_lo}-${fvg_hi}"
if ob:  details["ob"]  = f"OB ${ob_lo}-${ob_hi}"

details["score"] = score

# ── THRESHOLD CHECK ───────────────────────────────────
if score < SCORE_SETUP: return None

# ── UPGRADE 1: OVEREXTENSION FILTER ─────────────────
if len(p5) >= 10:
    move_size = abs(p5[-1] - p5[-10])
    atr_est   = sum([abs(p5[i]-p5[i-1]) for i in range(len(p5)-14,len(p5))]) / 14
    # Si el precio ya se movió más de 3x ATR → extendido
...
if len(p5) >= 4:
    # Simular velas con grupos de precios
    o = p5[-4]; c = p5[-1]
    h = max(p5[-4:]); l = min(p5[-4:])
    body = abs(c - o)
    if bias == "bullish":
        lower_wick = min(o,c) - l
        rejection_confirmed = lower_wick > body * 1.5  # pin bar alcista
    else:
        upper_wick = h - max(o,c)
        rejection_confirmed = upper_wick > body * 1.5  # pin bar bajista
...
# Overlap NY+LON = más volatilidad y calidad
if 13 <= h_utc <= 17:
    score += 5
    details["session_boost"] = "+5 Overlap"
# Sweep + rejection combo = señal muy fuerte
...
<style>
@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700&family=IBM+Plex+Mono:wght@300;400;500&family=Cormorant+Garamond:ital,wght@0,300;0,400;0,600;1,300;1,400&display=swap');:root {
–bg:     #080600;
–bg2:    #0C0A02;
–bg3:    #111005;
–bg4:    #161408;
...
</nav><!-- ══ DASHBOARD ══════════════════════════════════════════ --><div class="page active" id="page-dashboard">
<div class="main">
<div class="left"><div class="sessions-bar" id="sessionsBar">
  ...
  </div><!-- TIMEFRAME SELECTOR --><div style="display:flex;gap:6px;margin-bottom:16px;align-items:center">
  ...
  </div><div class="alert-box wait" id="alertBox">
  ...
  </div><!-- PANEL IA --><div class="ai-panel" id="aiPanel">
    <div class="ai-title">
  ...
  <button class="log-btn" id="logBtn" onclick="logCurrentSignal()" style="display:none">📊 REGISTRAR ESTA OPERACIÓN EN EXCEL</button><!-- TRADINGVIEW CHART --><div style="margin-bottom:20px">
...
</div><!-- ══ BACKTESTING ════════════════════════════════════════ --><div class="page" id="page-backtest">
<div class="bt-page">
  <div class="bt-header">
    <div>
...
</div><!-- ══ REGISTRO ═══════════════════════════════════════════ --><div class="page" id="page-registro">
<div class="reg-page">
...
</div><!-- ══ CONFIGURACIÓN ══════════════════════════════════════ --><div class="page" id="page-config">
<div class="bt-page">
...
window.addEventListener('resize',()=>{drawPriceChart();drawMACDChart()});
</script><script src="https://s3.tradingview.com/tv.js"></script><script>
  var _tvWidget = null;
  var _tvInterval = '5';
  ...
  function setTV(btn, interval) {
    ...
    btn.classList.add('active');
    initTVChart(interval);
  }

  // Init chart when page loads
  window.addEventListener('load', function() {
    setTimeout(() => initTVChart('5'), 800);
  });
  </script></body>
</html>"""class Handler(http.server.BaseHTTPRequestHandler):
def do_GET(self):
...
params = dict(urllib.parse.parse_qsl(urllib.parse.urlparse(self.path).query))
    ...
    if path == "/":
        ...
        if cached_news:
            ...
            news_data = {
                "active": news_on, "event": news_name or "",
        ...
        bias = get_bias_15m(_ict_prices_15m if len(_ict_prices_15m)>=12 else _ict_prices_5m)
        ...
        dataset_exists = os.path.exists(AurumAI.DATASET_FILE)
        dataset_size = 0
        if dataset_exists:
            try:
                with open(AurumAI.DATASET_FILE, "rb") as f:
        ...
        import time as _th
        lock_remaining = max(0, int((AurumAI._state_lock_until - _th.time()) / 60))
        self._send(200, "application/json", json.dumps({
            **health,
...
<defs>
  ...
  </defs><!-- Background --><rect width="600" height="340" fill="url(#bg)"/>
  <rect width="600" height="180" fill="url(#glow)"/><!-- Top border --><rect x="0" y="0" width="600" height="3" fill="{color_main}"/><!-- Border --><rect x="1" y="1" width="598" height="338" fill="none" stroke="{color_main}" stroke-width="1" stroke-opacity="0.4"/><!-- Corner accents --><line x1="1" y1="1" x2="40" y2="1" stroke="{color_gold}" stroke-width="1.5"/>
  ...
  <line x1="599" y1="339" x2="599" y2="300" stroke="{color_gold}" stroke-width="1.5"/><!-- AURUM logo --><text x="28" y="48" font-family="Georgia,serif" font-size="26" font-weight="bold" letter-spacing="8" fill="{color_gold}">AURUM</text>
<text x="30" y="64" font-family="monospace" font-size="9" letter-spacing="4" fill="#6B6550">SEÑALES XAU/USD · IA</text>
<!-- Live dot --><circle cx="558" cy="40" r="5" fill="{color_main}"/>
  <text x="568" y="44" font-family="monospace" font-size="9" letter-spacing="2" fill="#6B6550">VIVO</text><!-- Divider --><line x1="28" y1="78" x2="572" y2="78" stroke="{color_gold}" stroke-width="0.5" stroke-opacity="0.3"/>
  <line x1="28" y1="78" x2="120" y2="78" stroke="{color_gold}" stroke-width="1.5" stroke-opacity="0.8"/><!-- Signal label --><text x="28" y="102" font-family="monospace" font-size="9" letter-spacing="4" fill="#6B6550">SEÑAL VALIDADA · IA ACTIVA</text>
<!-- Big signal --><text x="26" y="168" font-family="Georgia,serif" font-size="64" font-weight="bold" letter-spacing="4" fill="{color_main}">{arrow}</text>
<!-- Price --><text x="28" y="200" font-family="monospace" font-size="11" letter-spacing="2" fill="#6B6550">PRECIO DE ENTRADA</text>
<text x="28" y="222" font-family="Georgia,serif" font-size="28" font-weight="bold" fill="#F0D080">${price:.2f}</text>
<!-- Divider 2 --><line x1="28" y1="238" x2="572" y2="238" stroke="{color_main}" stroke-width="0.5" stroke-opacity="0.3"/><!-- TP --><text x="28" y="262" font-family="monospace" font-size="9" letter-spacing="3" fill="#6B6550">TAKE PROFIT</text>
<text x="28" y="282" font-family="Georgia,serif" font-size="22" font-weight="bold" fill="{color_main}">${tp:.2f}</text>
<!-- SL --><text x="220" y="262" font-family="monospace" font-size="9" letter-spacing="3" fill="#6B6550">STOP LOSS</text>
<text x="220" y="282" font-family="Georgia,serif" font-size="22" font-weight="bold" fill="#FF4466">${sl:.2f}</text>
<!-- RR --><text x="410" y="262" font-family="monospace" font-size="9" letter-spacing="3" fill="#6B6550">RATIO R/B</text>
<text x="410" y="282" font-family="Georgia,serif" font-size="22" font-weight="bold" fill="{color_gold}">{rr}:1</text>
<!-- Bottom bar --><rect x="0" y="300" width="600" height="40" fill="#111109"/>
  <line x1="0" y1="300" x2="600" y2="300" stroke="{color_gold}" stroke-width="0.5" stroke-opacity="0.3"/><!-- Stats bottom --><text x="28" y="318" font-family="monospace" font-size="8" letter-spacing="2" fill="#6B6550">CONFIANZA</text>
...
print("\n  Para detener: Ctrl + C")
print("-" * 54)
# Threading server — maneja múltiples requests simultáneos
server = http.server.ThreadingHTTPServer(("0.0.0.0", PORT), Handler)
server.daemon_threads = True  # threads mueren con el proceso principal
