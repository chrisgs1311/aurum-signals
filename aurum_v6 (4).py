...
def _worker_price():
    """Worker #1: precio cada 3s con fallback entre APIs."""
    api_idx = 0
    while True:
        try:
...
def _worker_ohlc():
    ...
    intervals = {
        ...
        "4h":    {"tf": "4h",  "size": 30,  "freq": 1800},  # cada 30min
    }
    while True:
        try:
            now = time.time()
...
def start_all_workers():
    ...
    workers = [
        ("Price Feed (3s)",     _worker_price),
        ("OHLC Feed (3min)",     _worker_ohlc),
        ("Signal Engine (2s)",  _worker_signal),
...
def run_ict_engine(current_price, atr):
    """
    AURUM SCALP v6.1 — Motor principal.
    v6.1: sweep quality filter, reversa vs continuación, entrada temprana FVG
...
async function fetchAndAnalyze(){
  ...
  try{
    ...
    const [priceR,ohlcR,newsR,htfR,ictR]=await Promise.all([
      fetch('/precio').then(r=>r.json()).catch(()=>null),
      fetch('/ohlc?interval='+currentTF+'&size=200').then(r=>r.json()).catch(()=>[]),
      fetch('/newsstatus').then(r=>r.json()).catch(()=>({active:false})),
      fetch('/htftrend').then(r=>r.json()).catch(()=>({trend:'neutral'})),
      fetch('/ictsignal').then(r=>r.json()).catch(()=>null),
    ]);
    // OHLC
...
def main():
    ...
    for interval, tf, size in [("5min", "5m", 50), ("15min", "15m", 30), ("1h", "1h", 24), ("4h", "4h", 12)]:
        candles = get_historical_ohlc(interval, size)
        if candles:
