...
def run_ict_engine(current_price, atr):
    ...
    # ── RSI FILTER (no extremos) ──────────────────────────
    rsi = get_rsi_scalp(p5)
    if bias == "bullish" and rsi > 75: return None   # sobrecomprado
    if bias == "bearish" and rsi < 25: return None   # sobrevendido
...
function computeSignal(){
  ...
  checks.push({label:'RSI '+RSI.toFixed(0),ok:rok});if(!rok)valid=false;
